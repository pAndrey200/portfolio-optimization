...
log_handler = RotatingFileHandler("logs/app.log", maxBytes=5 * 1024 * 1024, backupCount=2)
...
def get_financial_news(query, api_key, max_results=3):
    ...
    url = (f"https://newsapi.org/v2/everything?q={query}&apiKey={api_key}&language=en&sortBy=publishedAt&domains={financial_domains}")
...
with tab1:
    ...
    if st.session_state.selected_ticker:
        ...
        try:
            ...
            period = st.selectbox("Выберите период:", ["1mo", "3mo", "6mo", "1y", "5y", "max"], index=2, key="analysis_period")
            interval = st.selectbox("Выберите интервал:", ["1d", "1wk", "1mo"], index=0, key="analysis_interval")
            ...
            if not data.empty:
                ...
                if news:
                    for idx, article in enumerate(news):
                        ...
                        st.write(f"*{article['source']['name']}* | Опубликовано: {article['publishedAt']}", key=f"news_source_{idx}")
                ...
                multi_tickers = st.text_input("Введите тикеры через запятую (например: AAPL, MSFT, TSLA):", "", key="correlation_input")
...
with tab2:
    ...
    if st.session_state.selected_ticker_forecast:
        ...
        try:
            stock = yf.Ticker(selected_ticker)
            period = st.selectbox("Выберите период данных:", ["1mo", "3mo", "6mo", "1y", "5y", "max"], index=4, key="forecast_period")
            steps = st.number_input("Введите количество шагов прогноза:", min_value=1, step=1, value=30, key="forecast_steps")
            ...
            if not data.empty:
                ...
                if adf_result[1] < 0.05:
                    st.write("Временной ряд стационарен.", key="adf_stationary")
                else:
                    st.write("Временной ряд нестационарен.", key="adf_nonstationary")

                st.header("Выберите модель для прогнозирования")
                model_type = st.selectbox("Тип модели", ["ARIMA", "SARIMA", "VAR"], key="forecast_model_type")

                if model_type == "VAR":
                    st.write("Будут использоваться переменные: High, Low, Close, Volume")
                    if st.button("Запустить модель VAR", key="forecast_var_button"):
                        logger.info("Запуск модели VAR")
                        data['High-Low'] = data['High'] - data['Low']
                        data = data.dropna()

                        model_data = data[['Close', 'Volume', 'High-Low']].diff().dropna()
                        var_model = VAR(model_data)
                        var_fit = var_model.fit(maxlags=5, ic='aic')

                        forecast_diff = var_fit.forecast(y=model_data.values[-var_fit.k_ar:], steps=steps)
                        ...
                        forecast.index = pd.date_range(start=data.index[-1], periods=steps, freq='D')

                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=data.index, y=data['Close'], mode='lines', name='Исторические данные'))
                        fig.add_trace(go.Scatter(x=forecast.index, y=forecast['Close'], mode='lines', name='Прогноз Close'))
                        fig.update_layout(
                    ...
                    use_auto = st.checkbox("Использовать автоматический подбор параметров", key="forecast_auto_checkbox")

                    if not use_auto:
                        p = st.number_input("Параметр p", min_value=0, step=1, value=1, key="forecast_p")
                        d = st.number_input("Параметр d", min_value=0, step=1, value=1, key="forecast_d")
                        q = st.number_input("Параметр q", min_value=0, step=1, value=1, key="forecast_q")
                        if model_type == "SARIMA":
                            P = st.number_input("Параметр P (сезонный)", min_value=0, step=1, value=1, key="forecast_P")
                            D = st.number_input("Параметр D (сезонный)", min_value=0, step=1, value=1, key="forecast_D")
                            Q = st.number_input("Параметр Q (сезонный)", min_value=0, step=1, value=1, key="forecast_Q")
                            m = st.number_input("Период сезонности", min_value=1, step=1, value=12, key="forecast_m")
                    ...
                    if st.button(f"Запустить модель {model_type}", key="forecast_model_button"):
                        ...
                        if use_auto:
                            ...
                            for p_try in range(0, 4):
                                for d_try in range(0, 2):
                                    for q_try in range(0, 4):
                                        try:
                                            temp_model = SARIMAX(data['Close'], order=(p_try, d_try, q_try)).fit(disp=False)
                            ...
                            st.write(f"Лучшие параметры: {best_order}, AIC: {best_aic}")
                            forecast = best_model.forecast(steps=steps)
                        else:
                            if model_type == "ARIMA":
                                model = SARIMAX(data['Close'], order=(p, d, q)).fit()
                            else:
                                model = SARIMAX(data['Close'], order=(p, d, q), seasonal_order=(P, D, Q, m)).fit()

                            forecast = model.forecast(steps=steps)

                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=data.index, y=data['Close'], mode='lines', name='Исторические данные'))
                        fig.add_trace(go.Scatter(x=pd.date_range(start=data.index[-1], periods=steps, freq='D'), y=forecast, mode='lines', name='Прогноз'))
                        fig.update_layout(
